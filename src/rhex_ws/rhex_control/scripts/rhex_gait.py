...
FREQUENCY = 100.0
STEP_SIZE = 1.5  # radians per step
STEP_TIME = 0.5  # minimum time per phase in seconds
...
class RHexTripodPIDController(Node):
    ...
    def joint_state_callback(self, msg: JointState):
        ...
        for name, pos in zip(msg.name, msg.position):
            if name in self.joint_angles:
                prev = self.joint_angles[name]
                self.joint_velocities[name] = (pos - prev) / dt
    ...
    def update(self):
        ...
        if self.linear_x != 0.0 or self.angular_z != 0.0:
                ...
                abs(self.target_angles[j] - self.joint_angles[j]) < 100
            ...
            if done and now - self.phase_start_time > 1.0:
                ...
                # Set new targets for active tripod
